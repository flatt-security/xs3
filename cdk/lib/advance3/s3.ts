...
export const uploadS3bucket = (scope: Construct, prefix: string, uniqueId: string): Bucket => {
  return new Bucket(scope, `${prefix}UploadBucket`, {
    bucketName: `${prefix.toLocaleLowerCase()}-${uniqueId.slice(0, 10)}-adv-3-upload`,
...
export const crawlingContentDeliveryS3bucket = (scope: Construct, prefix: string, uniqueId: string): Bucket => {
  return new Bucket(scope, `${prefix}DeliveryBucket`, {
    bucketName: `${prefix.toLocaleLowerCase()}-${uniqueId.slice(0, 10)}-adv-3-delivery`,
    removalPolicy: RemovalPolicy.DESTROY,
    encryption: BucketEncryption.S3_MANAGED,
    autoDeleteObjects: true,
    versioned: false,
...
export const specialFlagDeliveryS3bucket = (scope: Construct, prefix: string, uniqueId: string): Bucket => {
  const bucket = new Bucket(scope, `${prefix}SpecialFlagBucket`, {
    bucketName: `${prefix.toLocaleLowerCase()}-${uniqueId.slice(0, 10)}-adv3-special-flag`,
    removalPolicy: RemovalPolicy.DESTROY,
    encryption: BucketEncryption.S3_MANAGED,
    autoDeleteObjects: true,
    versioned: false,
  ...
  // Add Assets Files
  new BucketDeployment(scope, `${prefix}SpecialFlagBucketDeployment`, {
    sources: [Source.data('flag.txt', process.env.ADV_3_SP_FLAG || '')],
