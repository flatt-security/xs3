...
export const uploadS3bucket = (scope: Construct, prefix: string, uniqueId: string): Bucket => {
  return new Bucket(scope, `${prefix}UploadBucket`, {
    bucketName: `${prefix.toLocaleLowerCase()}-${uniqueId.slice(0, 10)}-adv1-upload`,
...
export const crawlingContentDeliveryS3bucket = (scope: Construct, prefix: string, uniqueId: string): Bucket => {
  return new Bucket(scope, `${prefix}DeliveryBucket`, {
    bucketName: `${prefix.toLocaleLowerCase()}-${uniqueId.slice(0, 10)}-adv1-delivery`,
    removalPolicy: RemovalPolicy.DESTROY,
    encryption: BucketEncryption.S3_MANAGED,
    autoDeleteObjects: true,
    versioned: false,
...
export const assetsDeliveryS3bucket = (scope: Construct, prefix: string, uniqueId: string): Bucket => {
  const bucket = new Bucket(scope, `${prefix}DeliveryBucket`, {
    bucketName: `${prefix.toLocaleLowerCase()}-${uniqueId.slice(0, 10)}-adv1-delivery`,
    removalPolicy: RemovalPolicy.DESTROY,
    encryption: BucketEncryption.S3_MANAGED,
    autoDeleteObjects: true,
  ...
  // Add Assets Files
  new BucketDeployment(scope, `${prefix}DeliveryBucketDeployment`, {
